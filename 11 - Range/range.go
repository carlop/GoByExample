...
func main() {
	...
	// entry. Above we didn't need the index, so we ignored it with the black
	// identifier _. Some times we actually want the indexes though.
	...
	// range can also itterate over just the keys of a map
	for k := range kvs {
		fmt.Println("key:", k)
	}
